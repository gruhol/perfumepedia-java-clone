...
public interface VoteNoteRepository extends JpaRepository<VoteNote, Long> {
    @Query(value = "SELECT SUM(vote_note.vote_value) as suma FROM `vote_note`" +
            ...
            nativeQuery = true)
    Integer getSumVoteToNote(@Param("idProduct") Long idProduct, @Param("idNote") Long idNote);

    VoteNote findByIdUserAndIdNoteAndIdProduct(Long idUser, Long idNote, Long idProduct);
}
