...
import org.perfumepedia.DataBase.service.VoteNoteService;
...
public class VoteProductRestController {
    ...
    @RequestMapping(value = "vote/votenote", method = RequestMethod.POST)
    public @ResponseBody ResponseAjax voteToNoteProduct(
            @RequestParam("idNote") Long idNote,
            @RequestParam("valueNote") Integer valueNote,
            @RequestParam("idProduct") Long idProduct)
    {
        String login = SecurityContextHolder.getContext().getAuthentication().getName();
        User user = userService.findUserByLogin(login);
        ...
        VoteNote existVoteNote = voteNoteService.findVoteNote(user.getIdUser(), idNote, idProduct);
        if (existVoteNote == null) {
            VoteNote voteNote = VoteNote.builder()
                    .idNote(idNote)
                    .idProduct(idProduct)
                    .voteValue(valueNote)
                    .idUser(user.getIdUser())
                    .build();
            voteNoteService.save(voteNote);
        } else {
            existVoteNote.setVoteValue(valueNote);
            voteNoteService.save(existVoteNote);
        }
        VoteNoteStat voteNoteStat = VoteNoteStat.builder()
                .idNote(idNote)
                .value(voteNoteService.getVoteNoteValueForProduct(idNote, idProduct))
                .build();
        return new ResponseAjax(DONE, voteNoteStat);
    }
